...
#Region "Constructors/Disposer"
        ...
        Public Sub New(connection As SqlConnection)
            'Connect to database with given connection
            myConn = connection
        ...
        Public Sub New(username As String, password As String)
            ...
            Me.New(New SqlConnectionStringBuilder(My.Settings.masterConnection) With {.UserID = username, .Password = password})
        End Sub
...
#Region "Listeners"
        ...
        Public Sub removeListener(email As String)
            myCmd.CommandText = String.Format("delete from email_listeners where email = '{0}'", email)

            myCmd.ExecuteNonQuery()
        End Sub

        Public Sub removeListener(listener As Listener)
            removeListener(listener.email)
        End Sub

        Public Sub updateListener(name As String, email As String)
            myCmd.CommandText = String.Format("update email_listeners set name='{0}', email='{1}' where email='{1}'", name, email)
